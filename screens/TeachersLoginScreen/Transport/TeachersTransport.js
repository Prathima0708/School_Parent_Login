...
const TransportScreen = () => {
  ...
  const [transportListData, setTransportListData] = useState([]);
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        //console.log(res.data[5].user_id.id)
        
        console.log('user id is',user
        )
      const selectedData=staffData.find((item) => item.user_id.id == userID)
       // console.log(selectedData)
        //setTransportListData(selectedData);
      } catch (error) {
        console.log(error);
      }
      // try {
