import Category from "./models";

export const CATEGORIES = [
  new Category(
    "c1",
    "Attendance",
    "#f5428d",
    "https://cdn-icons-png.flaticon.com/512/2/2123.png"
  ),
  new Category(
    "c2",
    "Fees",
    "#f54242",
    "https://img.icons8.com/external-flaticons-lineal-flat-icons/344/external-fees-university-flaticons-lineal-flat-icons.png"
  ),
  new Category(
    "c3",
    "Transport",
    "#f5a442",
    "https://img.icons8.com/carbon-copy/344/truck--v1.png"
  ),
  new Category(
    "c4",
    "Calender",
    "#f5d142",
    "https://img.icons8.com/external-yogi-aprelliyanto-basic-outline-yogi-aprelliyanto/344/external-calender-time-and-date-yogi-aprelliyanto-basic-outline-yogi-aprelliyanto.png"
  ),
  new Category(
    "c5",
    "Exam",
    "#368dff",
    "https://img.icons8.com/external-justicon-lineal-justicon/344/external-exam-back-to-school-justicon-lineal-justicon.png"
  ),
  new Category(
    "c6",
    "Marks Card",
    "#41d95d",
    "https://img.icons8.com/ios-filled/344/report-card.png"
  ),
  new Category(
    "c7",
    "Academics",
    "#9eecff",
    "https://img.icons8.com/external-stick-figures-gan-khoon-lay/344/external-academic-coach-instructor-trainer-teacher-jobs-occupations-careers-stick-figures-gan-khoon-lay.png"
  ),
  new Category(
    "c8",
    "Events",
    "#b9ffb0",
    "https://img.icons8.com/pastel-glyph/344/events--v1.png"
  ),
  new Category(
    "c9",
    "Leave",
    "#ffc7ff",
    "https://img.icons8.com/dotty/344/sign-up-in-calendar.png"
  ),
  new Category(
    "c10",
    "Homework",
    "#47fced",
    "https://img.icons8.com/external-itim2101-lineal-itim2101/344/external-homework-back-to-school-itim2101-lineal-itim2101.png"
  ),
];
